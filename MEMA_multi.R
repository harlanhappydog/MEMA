...
parameters {
  ...
  vector[Q+1] theta;
  corr_matrix[Q+1] C_tau;        // prior correlation
  vector<lower=0>[Q+1] d_tau;      // prior scale
}
...
transformed parameters {
  ...
  TAU_ = quad_form_diag(C_tau, d_tau); 
...
model {
  ...
  for (k in 1:NStudies)	  
	beta[k,] ~   multi_normal(theta, TAU_);
...
parameters {
  ...
  matrix[NStudies, Q+1] beta;
  vector[Q+1] theta;
  corr_matrix[Q+1] C_tau;        // prior correlation for TAU_
  vector<lower=0>[Q+1] d_tau;      // prior scale for TAU_
  corr_matrix[Q] C_lambda[NStudies];        // prior correlation for lambda
  vector<lower=0>[Q] d_lambda[NStudies];      // prior scale for lambda
}
...
transformed parameters {
  ...
  for(k in 1:NStudies){
	lambda[k] = quad_form_diag(C_lambda[k], d_lambda[k]);
	}	
}
...
model {
  ...
  for (k in 1:NStudies){	  
    ...
    d_lambda[k] ~ cauchy(0, 2.5);
    C_lambda[k] ~ lkj_corr(2);
