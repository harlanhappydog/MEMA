...
for(k in (1+round(length(tau)/3)):(2*round(length(tau)/3))){ tau[k] <- 2.47}
for(k in (1+2*round(length(tau)/3)):length(tau)){ tau[k] <- 2.47 }
...
#### BAYES MEMA3 (with no bounds for tau):
BayesMEMA3 <- 
"data {
  int<lower=0> NStudies;		
  vector<lower=0>[NStudies]  n_per_study;
