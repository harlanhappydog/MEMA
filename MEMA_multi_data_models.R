...
transformed parameters {
  ...
  for(k in 1:NStudies){lambda[k] = quad_form_diag(Omega_lambda[k], tau_lambda[k]);}	
}
...
model {
   ...
   for (j in 1:N){
   		...
   		X2[j,1] ~ normal(mu[studyID[j],2]*lambda[studyID[j]][1,2]*(1/lambda[studyID[j]][1,1])*(X1[j,1] -  mu[studyID[j],1]), sqrt(lambda[studyID[j]][2,2]-lambda[studyID[j]][1,2]*(1/lambda[studyID[j]][1,1])*lambda[studyID[j]][1,2] ));
		Y[j] ~   normal(append_col(1, X[j,])*(beta[studyID[j],]'), sigma[studyID[j]]);
	} 
